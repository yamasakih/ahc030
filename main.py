...
def debug_p(to_int: float = -1) -> None:
    if to_int == -1:
        for p in P:
...
def play_random() -> None:
    ...
    while times > 0:
        # 左上、右下の座標をランダムに求める
        width = randint(bottom, up)
        height = randint(bottom, up)
        left_up_x = randint(0, N - 1 - width)
        left_up_y = randint(0, N - 1 - height)
        right_bottom_x = left_up_x + width
        right_bottom_y = left_up_y + height
        # 占う
        times -= 1
        fields = []
